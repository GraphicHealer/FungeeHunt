<script lang="ts">
  ...
  // Per-device tour state. 'gmTourRun' === '1' means the tour is in progress;
  // 'gmTourStep' is the current step index. Set by the wizard's ask modal.
  let tourStep = 0;
  let running = false;
  let loading = false;
  ...
  const steps: any[] = [
    ...
    {
      title: 'New Game: Bonus Task',
      body: 'Step 4: optionally enable a limited-time bonus task for captains. Then click NEXT.',
      route: /^\/gm\/new$/,
      nextPath: null,
      target: '[data-tour="wizard-form"]',
      click: 'step4-next',
    ...
    },
    {
      ...
      body: 'Step 5: set food drive defaults if you like. Then click NEXT.',
      route: /^\/gm\/new$/,
      nextPath: null,
      target: '[data-tour="wizard-form"]',
      click: 'step5-next',
    ...
    {
      title: 'New Game: Email',
      body: 'Step 6: enter your email to get the game link, a start reminder, and a deletion warning. Then click CREATE GAME.',
      route: /^\/gm\/new$/,
      nextPath: null,
      target: '[data-tour="wizard-form"]',
      click: 'create-game',
  ...
  $: visible = running && currentStep && currentStep.route.test(currentPath);

  function load() {
    running = localStorage.getItem('gmTourRun') === '1';
    tourStep = Number(localStorage.getItem('gmTourStep') ?? '0') || 0;
    maybeAutoAdvance();
    positionBubble();
  }

  function maybeAutoAdvance() {
    if (!running || !currentStep || !currentStep.nextPath) return;
    if (resolvePath(currentStep.nextPath) === currentPath && tourStep < steps.length - 1) {
      setStep(tourStep + 1, false);
    }
  }

  function setStep(step: number, done: boolean) {
    loading = true;
    tourStep = step;
    if (done) {
      localStorage.removeItem('gmTourRun');
      localStorage.removeItem('gmTourStep');
      localStorage.setItem('gmTourPref', 'no');
      running = false;
    } else {
      localStorage.setItem('gmTourStep', String(step));
    }
    loading = false;
  }
  ...
  async function onGlobalClick(e: MouseEvent) {
    if (!running || loading || !currentStep) return;
  ...
  onMount(() => {
    ...
    window.addEventListener('gm-tour-start', load);
    load();
    poll = setInterval(() => {
  ...
  onDestroy(() => {
    ...
    window.removeEventListener('gm-tour-start', load);
